...
select_clause
   : SELECT (val_distinct)? column_list_clause ( FROM table_references )? ( where_clause )?
   ;
...
column_name
   : ( col_compress )? column_value ( AS column_name_alias )?
   ;
...
aggegrate
   ...
   column_value
   : ( ( schema_name DOT )? ID DOT )? ID | aggegrate LPAREN aggegrate_column_name RPAREN | ( ( schema_name DOT)? ID DOT)? ASTERISK
   ;
...
xml_column_name
	: xml_column_value (AS xml_column_alias)? | xml_block
	;
...
xml_block
	: LTH ( xml_compress )? xml_tag COMMA xml_list_clause COMMA GTH (COMMA)?
	;

column_block
    : column_name | xml_block
	;

xml_list_clause
	: xml_column_name (COMMA xml_column_name)*
	;
...
column_list_clause
   : column_block ( COMMA column_block )*
   ;
...
element
   : USER_VAR | ID | ( STRING_LITERAL ) | INT | element_column_name
   ;
